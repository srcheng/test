        ...
        private void UpdateLED()
        {
            ...
            if (controller.Touch1Active)
            {
                ...
                if (_lastLEDindex != index)
                {
                    ...
                    if (controller.TouchpadGesture.Type == MLInputControllerTouchpadGestureType.Swipe)
                    {
                        _color = MLInputControllerFeedbackColorLED.BrightCosmicPurple;
                    }
                    else if (controller.TouchpadGesture.Type == MLInputControllerTouchpadGestureType.Tap)
                    {
                        _color = MLInputControllerFeedbackColorLED.BrightCelestialBlue;
                    }
                    else if (controller.TouchpadGesture.Type == MLInputControllerTouchpadGestureType.RadialScroll)
                    {
                        _color = MLInputControllerFeedbackColorLED.BrightMissionRed;
                    }
                    else if (controller.TouchpadGesture.Type == MLInputControllerTouchpadGestureType.SecondForceDown)
                    {
                        _color = MLInputControllerFeedbackColorLED.PastelFloridaOrange;
                    }

                    controller.StartFeedbackPatternLED((MLInputControllerFeedbackPatternLED)index, _color, 0);
                    _lastLEDindex = index;
                }
            }
